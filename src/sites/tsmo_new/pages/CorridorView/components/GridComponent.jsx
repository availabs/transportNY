...
const GridComponent = ({ date, data, ttToSpeed, TMCs, tmcWidths, scale, onClick, isVisible = true }) => {
  ...
  React.useEffect(() => {
    ...
    startLoading();
    falcor.get(["transcom3", TSMO_VIEW_ID, "events", "tmc", TMCs, date, ["Incident", "Construction"]])
      .then(res => {
        const numEvents = TMCs.reduce((a, c) => {
          return ["Incident", "Construction"].reduce((aa, cc) => {
            const n = get(res, ["json", "transcom3", TSMO_VIEW_ID, "events", "tmc", c, date, cc, "length"], 0);
            return aa + n;
          }, a);
        }, 0);
        dispatch({
  ...
  React.useEffect(() => {
    ...
    const events = TMCs.reduce((a, c) => {
      return ["Incident", "Construction"].reduce((aa, cc) => {
        const e = get(falcorCache, ["transcom3", TSMO_VIEW_ID, "events", "tmc", c, date, cc, "value"], []);
        aa.push(...e);
        return aa;
      }, a);
    }, []);
