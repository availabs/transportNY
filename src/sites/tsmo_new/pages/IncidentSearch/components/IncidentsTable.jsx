...
const IncidentsTable = ({ events, setHoveredEvent }) => {
  ...
  let Events = React.useMemo(() => {
    falcor.chunk([
      "transcom3", TSMO_VIEW_ID, "eventsbyId", eventsIds,
      ["event_id",
        "n",
        "congestion_data",
        "facility",
    ...
    const facility = eventsIds.map((eventId, i) => {
      const eventFacility = get(falcorCache, ["transcom3", TSMO_VIEW_ID, "eventsbyId", eventId], null);
      return {
    ...
    const eventsData = eventsIds.map((eventId, i) => {
      const eventData = get(falcorCache, ["transcom3", TSMO_VIEW_ID, "eventsbyId", eventId], null);
      const delay_cost = get(falcorCache, ["transcom3", TSMO_VIEW_ID,"eventsbyId", eventId, "congestion_data", "value"], null);
