							...
							.then(res => {
								...
								if (group === "indices") {
									INDICES.forEach(index => {
										...
										route.data[index.key] = get(res, getDataPath, []) //get(res, `json.routes.npmrds.view.${Number(3639)}.data.${ key }`, [])
									...
									TMC_ATTRIBUTES.forEach(att => {
										route.data[att.key] =  get(res, getDataPath, []) //get(res, `json.routes.npmrds.view.${Number(3639)}.data.${ key }`, [])
											.map(d => ({ tmc: d.tmc, value: d[att.alias || att.key] }))
											.filter(({ value }) => value !== undefined)
									})
								}
								else if (group === "hoursOfDelay") {
									route.data["hoursOfDelay"] =  get(res, getDataPath, []) //get(res, `json.routes.npmrds.view.${Number(3639)}.data.${ key }`, [])
										.map(d => ({ ...d, value: d["hoursOfDelay"] }));
									route.data["avgHoursOfDelay"] =  get(res, getDataPath, []) //get(res, `json.routes.npmrds.view.${Number(3639)}.data.${ key }`, [])
										.map(d => ({ ...d, value: d["avgHoursOfDelay"] }));
								}
								else if (group === "co2Emissions") {
									route.data["co2Emissions"] =  get(res, getDataPath, []) //get(res, `json.routes.npmrds.view.${Number(3639)}.data.${ key }`, [])
										.map(d => ({ ...d, value: d["co2Emissions"] }));
									route.data["avgCo2Emissions"] =  get(res, getDataPath, []) //get(res, `json.routes.npmrds.view.${Number(3639)}.data.${ key }`, [])
										.map(d => ({ ...d, value: d["avgCo2Emissions"] }));
								}
								else {
									route.data[dd] =  get(res, getDataPath, []) //get(res, `json.routes.npmrds.view.${Number(3639)}.data.${ key }`, []);
									if (alias) {
										route.data[alias] =  get(res, getDataPath, []) //get(res, `json.routes.npmrds.view.${Number(3639)}.data.${ key }`, []);
									}
								}
// console.log('GeneralGraphComp::route.data',route.data, year);
							})
						})
					}, Promise.resolve())
				})
