							...
							.then(res => {
								if (group === "indices") {
									...
									TMC_ATTRIBUTES.forEach(att => {
										route.data[att.key] =  get(res, `json.routes.data.${ key }`, []) //get(res, `json.routes.npmrds.view.${Number(3639)}.data.${ key }`, [])
											.map(d => ({ tmc: d.tmc, value: d[att.alias || att.key] }))
											.filter(({ value }) => value !== undefined)
									})
								}
								else if (group === "hoursOfDelay") {
									route.data["hoursOfDelay"] =  get(res, `json.routes.data.${ key }`, []) //get(res, `json.routes.npmrds.view.${Number(3639)}.data.${ key }`, [])
										.map(d => ({ ...d, value: d["hoursOfDelay"] }));
									route.data["avgHoursOfDelay"] =  get(res, `json.routes.data.${ key }`, []) //get(res, `json.routes.npmrds.view.${Number(3639)}.data.${ key }`, [])
										.map(d => ({ ...d, value: d["avgHoursOfDelay"] }));
								}
								else if (group === "co2Emissions") {
									route.data["co2Emissions"] =  get(res, `json.routes.data.${ key }`, []) //get(res, `json.routes.npmrds.view.${Number(3639)}.data.${ key }`, [])
										.map(d => ({ ...d, value: d["co2Emissions"] }));
									route.data["avgCo2Emissions"] =  get(res, `json.routes.data.${ key }`, []) //get(res, `json.routes.npmrds.view.${Number(3639)}.data.${ key }`, [])
										.map(d => ({ ...d, value: d["avgCo2Emissions"] }));
								}
								else {
									route.data[dd] =  get(res, `json.routes.data.${ key }`, []) //get(res, `json.routes.npmrds.view.${Number(3639)}.data.${ key }`, []);
									if (alias) {
										route.data[alias] =  get(res, `json.routes.data.${ key }`, []) //get(res, `json.routes.npmrds.view.${Number(3639)}.data.${ key }`, []);
									}
								}
							})
						})
