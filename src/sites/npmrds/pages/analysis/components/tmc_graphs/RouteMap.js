...
class RouteMap extends HybridGraphComp {
	...
	componentDidUpdate(oldProps, oldState) {
		...
		if (this.state.map && !deepequal(bounds, this.state.bounds)) {
			this.setState({ bounds });
		}
		if (!deepequal(oldState.bounds, this.state.bounds)) {
			this.fitBounds();
		}

		if (this.state.map &&
				!deepequal(oldProps.highlightedTmcs, this.props.highlightedTmcs)) {
		}
	}
	resize() {
	...
	onMouseMove({ features, point }, ...rest) {
		...
		if (!deepequal(tmcs, this.props.highlightedTmcs)) {
		...
		if (newComp) {
			activeRouteComponents = activeRouteComponents.filter(compId => {
				...
				return !current.length ||
					(compId === newCompId) ||
					!deepequal(newComp.tmcArray, comp.tmcArray)
			})
		}
