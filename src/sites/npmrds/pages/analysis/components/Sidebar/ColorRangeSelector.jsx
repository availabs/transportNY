...
import { Control } from "./components/parts"
      ...
      <div>
        { colors.map(c =>
            <ColorRange key={ c.name } colors={ reversed ? c.reversed : c.colors }
              isSelected={ isEqual(reversed ? c.reversed : c.colors, colorRange) }
...
export default class ColorRangeSelector extends React.Component {
  ...
  componentDidMount() {
    for (const type in COLORS) {
      for (const range of COLORS[type].colors[this.state.size]) {
        if (isEqual(range.colors, this.props.colorRange)) {
          return this.setState({ type, reversed: false });
        }
        else if (isEqual(range.reversed, this.props.colorRange)) {
  ...
  componentDidUpdate(oldProps) {
    if (!isEqual(oldProps.colorRange, this.props.colorRange)) {
      this.componentDidMount();
    }
  }
  render() {
    const usingDefault = this.props.defaultColorRange.length && isEqual(this.props.colorRange, this.props.defaultColorRange)
    return (
      <StyledColorSelector>
