...
export default function Manage({
    ...
    const batches = useMemo(() => source?.metadata?.batches, [source]);
    ...
    const geomView = useMemo(() => {
        return (npmrds_production_source_id && (Object.values(get(falcorCache, ["dama", pgEnv, "sources", "byId", npmrds_production_source_id, "views", "byIndex"], {}))
            ...
            .filter(f => f.metadata?.start_date && f?.metadata?.end_date) || [])[0];
    }, [falcorCache, pgEnv]);
    ...
    const update = async () => {
        ...
        setLoading(true);
        try {
            const res = await fetch(`${DAMA_HOST}/dama-admin/${pgEnv}/npmrds/transcom_congestion/add`, {
                method: "POST",
    ...
    const remove = async (view_id) => {
        ...
        try {
            const res = await fetch(
                `${DAMA_HOST}/dama-admin/${pgEnv}/npmrds/transcom_congestion/remove`,
                {
                    method: "POST",
                    body: JSON.stringify(publishData),
                    headers: {
                ...
                <div className="overflow-x-auto px-5 py-3">
                    <table className="min-w-full bg-white">
                        ...
                        <tbody>
                            ...
                            {Object.keys(batches || {}).map((item, index) => (
                                <tr key={index}>
                                    <td
                                        key={`${item}`}
                                    ...
                                    <td
                                        ...
                                        className="py-2 px-4 border-b"
                                    >
                                        {batches[item] && batches[item][0]}
                                    </td>
                                    <td
                                        key={`${batches[item][1]}`}
                                        className="py-2 px-4 border-b"
                                    >
                                        {batches[item] && batches[item][1]}
                                    </td>

                                    {index === 0 ||
                                        index === Object.keys(batches || {}).length - 1 ? (
                                        <>
                                            <td
                                                key={`${index}`}
                                                className="py-2 px-4 border-b"
                        ...
                        <div className="inline-block w-full max-w-md p-6 my-8 text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
                            ...
                            <div className="mt-4">
                                ...
                                <button
                                    ...
                                    className="ml-3 inline-flex justify-center px-4 py-2 text-sm text-green-900 bg-green-100 border border-transparent rounded-md hover:bg-green-200 duration-300"
                                    onClick={async () => {
                                        await remove(Number(activeViewId));
